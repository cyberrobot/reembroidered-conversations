...
test("partially omitted ranges use server defaults while preserving explicit bounds", async () => {
  ...
  assert.equal(
    (
      await handler(
        new Request(`http://localhost/api/availability?from=${tomorrow}`),
      )
    ).status,
    200,
  );
  assert.equal(
    (
      await handler(
        new Request(`http://localhost/api/availability?to=${tomorrow}`),
      )
    ).status,
    200,
  );
...
test("booking UI requests server-default availability without encoding a horizon", async () => {
  const hook = await readFile(
    new URL("../src/hooks/useAvailability.ts", import.meta.url),
    "utf8",
  );
  const booking = await readFile(
    new URL("../src/components/BookingSection.tsx", import.meta.url),
    "utf8",
  );
...
test("malformed, reversed, past, and excessive ranges return one stable 400 error", async () => {
  const shortHorizonConfig = {
    ...PROVIDER_AVAILABILITY_CONFIG,
    maximumBookingHorizonDays: 2,
  };
  ...
  for (const invalidRequest of scenarios) {
    ...
    const response = await createHandler(async () => {
      called = true;
      return [];
    }, shortHorizonConfig)(invalidRequest);
...
test("dependency and reauthorization failures map to the same stable safe 503 response", async () => {
  for (const error of [
    ...
    Object.assign(new Error("oauth detail"), {
