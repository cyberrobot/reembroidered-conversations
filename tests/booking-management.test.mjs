...
test("invalid capability fails before persistence and reveals no booking state", async () => {
  let lookups = 0;
  const state = await getBookingManagementState(
    `${bookingId}.tampered`,
    new Date(),
    {
      secret,
...
test("valid capability returns a narrow active view with server-owned policy and product data", async () => {
    ...
    new Date("2030-01-01T10:00:00.000Z"),
    {
      ...
      persistence: {
        findBooking: async () => ({
          ...
          stripePaymentIntentId: "pi_must_not_be_returned",
          calendarEventId: "event_must_not_be_returned",
          cancelledAt: null,
  ...
  assert.equal(
    JSON.stringify(state).includes("pi_must_not_be_returned"),
    false,
  );
  assert.equal(
    JSON.stringify(state).includes("event_must_not_be_returned"),
    false,
  );
...
test("a linked replacement hold is exposed as a resumable pending change", async () => {
    ...
    new Date("2030-01-01T10:00:00.000Z"),
    {
      ...
      persistence: {
        findBooking: async () => ({
          ...
          rescheduleHold: {
            status: "HOLD",
            startAt: new Date("2030-01-04T12:00:00.000Z"),
            endAt: new Date("2030-01-04T12:55:00.000Z"),
            timezone: "Europe/London",
          },
        }),
      },
  ...
  assert.deepEqual(state.target, {
    startAt: "2030-01-04T12:00:00.000Z",
    endAt: "2030-01-04T12:55:00.000Z",
    timezone: "Europe/London",
  });
});
