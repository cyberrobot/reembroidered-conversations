...
function persistence(
  ...
  let booking = {
    id: bookingId,
    name: "Persisted Listener",
    email: "listener@example.com",
    startAt: new Date("2030-01-01T10:00:00Z"),
    endAt: new Date("2030-01-01T10:55:00Z"),
...
test("email failure leaves booking CONFIRMED and replay retries until acceptance, then skips duplicates", async () => {
  ...
  await processStripeWebhookEvent(
    event("checkout.session.completed"),
    p,
    async () => assert.fail("Calendar must not repeat"),
    send,
  );
  await processStripeWebhookEvent(
    event("checkout.session.completed"),
    ...
    await assert.rejects(
      () =>
        processStripeWebhookEvent(
          ...
          async () => assert.fail("Calendar must not repeat"),
          async () => {
            throw new EmailDeliveryError(providerCode);
          },
        ),
      (error) =>
        error instanceof StripeWebhookReconciliationError &&
        error.code === reconciliationCode,
    );
    assert.equal(p.get().status, "CONFIRMED");
    assert.equal(p.get().confirmationEmailSentAt, null);
  }
...
test("remote email acceptance followed by local persistence failure recovers idempotently", async () => {
  ...
  const send = (confirmedBooking) =>
    sendBookingConfirmationEmail(confirmedBooking, {
      configuration: {
        apiKey: "test-key",
        from: "Bookings <bookings@example.test>",
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof StripeWebhookReconciliationError &&
      error.code === "email_persistence_pending",
  );
  assert.equal(p.get().status, "CONFIRMED");
...
test("already-recorded confirmation email is idempotent on webhook replay", async () => {
  const p = persistence({
    ...
    calendarEventId,
    meetingUrl,
    confirmationEmailSentAt: new Date("2030-01-01T11:00:00Z"),
    ...
    await assert.rejects(
      () =>
        processStripeWebhookEvent(
          ...
          async () => assert.fail("Calendar finalization must not run"),
        ),
      StripeWebhookReconciliationError,
    );
  }
...
test("remote success followed by local persistence failure recovers the same event and Meet on webhook retry", async () => {
  ...
  let getAttempts = 0;
  const finalize = (paidBooking) =>
    reconcileBookingCalendarEvent(paidBooking, {
  ...
  await assert.rejects(
    () =>
      processStripeWebhookEvent(
        ...
        persistence({
          status: "HOLD",
          stripeCheckoutSessionId: "different",
          stripePaymentIntentId: null,
        }),
      ),
    StripeWebhookReconciliationError,
...
test("expiry never downgrades paid and completion never downgrades confirmed", async () => {
  const paid = persistence({
    status: "PAID",
    stripeCheckoutSessionId: "cs_test_one",
    stripePaymentIntentId: "pi_test_one",
  });
  await processStripeWebhookEvent(
    event("checkout.session.expired", { payment_status: "unpaid" }),
    paid,
  );
