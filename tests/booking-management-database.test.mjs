...
function confirmedData(overrides = {}) {
  return {
    ...
    email: "database-listener@example.test",
    startAt: originalStart,
    endAt: originalEnd,
    timezone: "Europe/London",
    status: "CONFIRMED",
    stripeCheckoutSessionId: "cs_management_db",
...
test(
  "authoritative cancellation releases the active slot and persists the refund decision",
  { skip },
  async () => {
    ...
    try {
      await db.booking.deleteMany({
        where: { id: { in: [sourceId, holdId, competingId] } },
      });
      ...
      const replacement = await db.booking.create({
        data: {
          ...
          timezone: "Europe/London",
          status: "HOLD",
          expiresAt: new Date("2042-01-02T10:15:00.000Z"),
          createdAt: new Date("2042-01-02T10:00:00.000Z"),
        },
      });
      assert.equal(
        replacement.startAt.toISOString(),
...
test(
  "reschedule hold blocks both slots and atomic transfer keeps target continuously owned",
  { skip },
  async () => {
    const db = new PrismaClient({
      adapter: new PrismaPg({ connectionString }),
    ...
    try {
      ...
      assert.equal(
        await db.booking.count({
          where: {
            status: { in: ["HOLD", "PAID", "CONFIRMED"] },
            startAt: { in: [originalStart, targetStart] },
          },
        }),
        2,
      );
      ...
      const result = await persistence.commitTarget({
        ...
        now: new Date("2042-01-02T10:01:00.000Z"),
      });
      ...
      await db.booking.update({
        where: { id: sourceId },
        data: { startAt: originalStart, endAt: originalEnd },
      });
      await db.booking.update({
        where: { id: hold.id },
        data: {
      ...
      let retiredInside;
      const retiredSignal = new Promise((resolve) => {
        retiredInside = resolve;
      });
      const continueSignal = new Promise((resolve) => {
      ...
      const swap = db.$transaction(async (transaction) => {
        ...
        await transaction.booking.update({
          where: { id: sourceId },
          data: { startAt: targetStart, endAt: targetEnd },
        });
      });
      await retiredSignal;
      let contenderSettled = false;
      const contender = competitor.booking
        .create({
          data: {
      ...
      assert.equal(
        contenderSettled,
        false,
        "target must not be externally observable as free",
      );
      continueTransaction();
      await swap;
      await assert.rejects(contender, (error) =>
        isActiveSlotUniqueConflict(error),
      );
    } finally {
      await db.booking.deleteMany({
        where: { id: { in: [sourceId, holdId, competingId] } },
      });
      await competitor.booking.deleteMany({
