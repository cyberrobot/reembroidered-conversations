...
function rejectsWithCode(code) {
  return (error) =>
    error instanceof CalendarAvailabilityError && error.code === code;
}
  ...
  await assert.rejects(
    () =>
      getBusyPeriods(
        ...
        serviceDependencies({
          getCredentials: async () =>
            credentials({ grantedScopes: ["openid", "email"] }),
          refreshAccessToken: callGoogle,
        }),
      ),
      ...
      return Response.json({
        ...
        calendars: {
          "primary@example.com": {
            busy: [
              {
                start: "2030-01-01T12:30:00+01:00",
                end: "2030-01-01T13:15:00+01:00",
              },
...
test("FreeBusy returns an empty array only for a valid empty calendar result", async () => {
  const result = await queryGoogleFreeBusy(
    ...
    async () =>
      Response.json({ calendars: { "primary@example.com": { busy: [] } } }),
  );
  assert.deepEqual(result, []);
});
...
test("FreeBusy treats a calendar-level error as provider failure, never empty availability", async () => {
  await assert.rejects(
    ...
    (error) =>
      error instanceof GoogleApiError && error.category === "unavailable",
  );
});
...
test("FreeBusy rejects every malformed or partial successful response", async () => {
  ...
  const invalidBodies = [
    ...
    { calendars: { "primary@example.com": { busy: [{}] } } },
    {
      calendars: {
        "primary@example.com": {
          busy: [{ start: "2030-01-01", end: "2030-01-02" }],
        },
      },
    },
    {
      calendars: {
        "primary@example.com": {
          busy: [{ start: "invalid", end: "2030-01-01T11:00:00Z" }],
        },
      },
    },
    {
      calendars: {
        "primary@example.com": {
          busy: [{ start: "2030-01-01T10:00:00Z", end: "invalid" }],
        },
      },
    },
    {
      calendars: {
        "primary@example.com": {
          busy: [
            { start: "2030-01-01T10:00:00Z", end: "2030-01-01T10:00:00Z" },
          ],
        },
      },
    },
    {
      calendars: {
        "primary@example.com": {
          busy: [
            { start: "2030-01-01T11:00:00Z", end: "2030-01-01T10:00:00Z" },
          ],
        },
      },
    },
  ...
  for (const body of invalidBodies) {
    await assert.rejects(
      () =>
        queryGoogleFreeBusy(input, async () =>
          body === undefined ? new Response("not-json") : Response.json(body),
        ),
      (error) =>
        error instanceof GoogleApiError &&
