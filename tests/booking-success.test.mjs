...
import {
  formatConfirmedBooking,
  getBookingSuccessState,
} from "../src/lib/booking/booking-success.mjs";
...
test("invalid identifiers fail before persistence and reveal no booking data", async () => {
  let calls = 0;
  const dependency = {
    persistence: persistence(booking(), () => {
      calls += 1;
    }),
  };
  for (const input of [
    ...
    assert.deepEqual(await getBookingSuccessState(input, dependency), {
      kind: "invalid",
    });
...
test("missing and mismatched bookings use the same generic invalid state", async () => {
  ...
  const result = await getBookingSuccessState(
    { bookingId, sessionId: "cs_test_attacker" },
    { persistence: persistence(booking()) },
  );
...
test("persisted lifecycle maps to truthful presentation states", async () => {
  for (const [status, kind] of [
    ["HOLD", "confirming-payment"],
    ["PAID", "finalising-booking"],
    ["CANCELLED", "inactive"],
    ["REFUNDED", "inactive"],
  ]) {
    assert.deepEqual(
      await getBookingSuccessState(
        { bookingId, sessionId },
        { persistence: persistence(booking({ status })) },
      ),
      { kind },
    );
  }
...
test("confirmed rows with incomplete finalisation or unsafe Meet URLs fail closed", async () => {
  for (const overrides of [
    ...
    assert.deepEqual(
      await getBookingSuccessState(
        { bookingId, sessionId },
        { persistence: persistence(booking(overrides)) },
      ),
      { kind: "unavailable" },
    );
  }
...
test("time formatting follows the persisted timezone across London daylight saving", () => {
  ...
  const winter = formatConfirmedBooking(
    booking({
      startAt: new Date("2026-12-03T14:00:00.000Z"),
      endAt: new Date("2026-12-03T14:55:00.000Z"),
    }),
  );
