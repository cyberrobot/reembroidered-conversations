...
async function realDependencies(overrides = {}) {
  const { getProviderCandidateSlotsForDate } =
    await import("../src/lib/availability/candidate-slots.mjs");
  return dependencies({
...
test("no conflicts leaves canonical candidates available using one bounded query per dependency", async () => {
  ...
  const deps = await realDependencies({
    getBookingConflicts: async (range) => {
      calls.push(["database", range]);
      return [];
    },
    getCalendarBusyPeriods: async (from, to) => {
      calls.push(["google", { from, to }]);
      return [];
    },
  });
  const result = await getAvailableSlots(
    { fromDate: date, toDate: date, now },
    deps,
  );
...
test("Google overlap removes occupancy but a boundary-touching period does not", async () => {
  const deps = await realDependencies({
    getCalendarBusyPeriods: async () => [
      {
        startAt: "2026-09-07T09:54:00.000Z",
        endAt: "2026-09-07T09:55:00.000Z",
      },
      {
        startAt: "2026-09-07T10:50:00.000Z",
  ...
  assert.equal(
    result.some((slot) => slot.startAt === "2026-09-07T09:00:00.000Z"),
    false,
  );
  assert.equal(
    result.some((slot) => slot.startAt === "2026-09-07T09:55:00.000Z"),
    true,
  );
  assert.equal(
    intervalsOverlap(
    ...
    getBookingConflicts: async () => [
      {
        ...
        rescheduleSourceBookingId: "5a449655-7be3-432c-a124-b769e10b50ef",
      },
    ],
  });
  const result = await getAvailableSlots(
...
test("booking and candidate occupancy buffers remove otherwise non-overlapping sessions", async () => {
  ...
  const result = await getAvailableSlots(
    { fromDate: date, toDate: date, now, config },
    deps,
  );
  assert.equal(
    result.some((slot) => slot.startAt === "2026-09-07T09:10:00.000Z"),
    false,
  );
});
...
test("multiple conflict sources leave only genuinely available sorted unique slots", async () => {
  ...
  assert.deepEqual(
    result,
    [...new Map(result.map((slot) => [slot.startAt, slot])).values()].sort(
...
test("duplicate candidate output is normalized into chronological unique slots", async () => {
  const later = {
    ...
    endAt: "2026-09-07T10:55:00.000Z",
    occupancyStartAt: "2026-09-07T10:00:00.000Z",
    occupancyEndAt: "2026-09-07T10:55:00.000Z",
  };
  const earlier = {
    startAt: "2026-09-07T09:00:00.000Z",
    endAt: "2026-09-07T09:55:00.000Z",
    occupancyStartAt: "2026-09-07T09:00:00.000Z",
    occupancyEndAt: "2026-09-07T09:55:00.000Z",
  };
  const result = await getAvailableSlots(
    { fromDate: date, toDate: date, now },
    dependencies({
...
test("empty provider range performs no database or Google work", async () => {
  ...
  assert.deepEqual(
    await getAvailableSlots(
      { fromDate: "2026-09-12", toDate: "2026-09-13", now },
      deps,
    ),
    [],
  );
...
test("database and Google failures propagate instead of becoming free slots", async () => {
  for (const failing of ["database", "google"]) {
    ...
    await assert.rejects(() =>
      getAvailableSlots({ fromDate: date, toDate: date, now }, deps),
    );
  }
});
...
test("provider-local candidates remain deterministic across London DST", async () => {
  ...
  const summer = await getAvailableSlots(
    {
      fromDate: "2026-07-06",
      toDate: "2026-07-06",
