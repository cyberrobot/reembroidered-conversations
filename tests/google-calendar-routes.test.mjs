...
const sessionModule = new URL("../src/lib/admin/session.ts", import.meta.url)
  .href;
...
process.env.ADMIN_SESSION_SECRET =
  "route-test-session-secret-that-is-at-least-32-bytes";

const { GET: connectRoute } =
  await import("../src/app/api/admin/google-calendar/connect/route.ts");
const { GET: callbackRoute } =
  await import("../src/app/api/admin/google-calendar/callback/route.ts");
...
function createIdentityToken(email = "admin@example.com", overrides = {}) {
  const header = Buffer.from(
    JSON.stringify({ alg: "RS256", kid: "route-test-key" }),
  ).toString("base64url");
...
function callbackRequest(
  transaction,
  query = {},
  cookieHeader = transaction.cookieHeader,
) {
  const url = new URL(
...
function installGoogleFetch(scenario = {}) {
  ...
  globalThis.fetch = async (input, init = {}) => {
    ...
    if (url.toString() === "https://oauth2.googleapis.com/token") {
      if (scenario.tokenFailure)
        return new Response("sensitive token failure", { status: 500 });
      return Response.json({
        access_token: "temporary-access-token",
        refresh_token: scenario.missingRefreshToken
          ? undefined
        ...
        scope:
          "openid email https://www.googleapis.com/auth/calendar.calendarlist.readonly https://www.googleapis.com/auth/calendar.freebusy https://www.googleapis.com/auth/calendar.events.owned",
      });
    }
    ...
    if (
      ...
      if (scenario.calendarFailure)
        return new Response("sensitive calendar failure", { status: 503 });
      if (scenario.noPrimary)
        return Response.json({
...
test("connect route emits the exact secure authorization request without persistence", async () => {
  ...
  assert.equal(
    transaction.authorizationUrl.origin,
    "https://accounts.google.com",
  );
  assert.equal(
    transaction.authorizationUrl.searchParams.get("response_type"),
    "code",
  );
  assert.equal(
    transaction.authorizationUrl.searchParams.get("access_type"),
    "offline",
  );
  assert.equal(
    transaction.authorizationUrl.searchParams.get("prompt"),
    "consent",
  );
  assert.equal(
    transaction.authorizationUrl.searchParams.get("code_challenge_method"),
    "S256",
  );
  assert.ok(transaction.state);
  assert.ok(transaction.authorizationUrl.searchParams.get("code_challenge"));
  assert.deepEqual(
    transaction.authorizationUrl.searchParams.get("scope").split(" "),
    [
      "openid",
      "email",
      "https://www.googleapis.com/auth/calendar.calendarlist.readonly",
  ...
  assert.doesNotMatch(
    developmentResponse.headers.getSetCookie().join("\n"),
    /; Secure/i,
  );
});
...
test("callback route completes the real orchestration and rejects replay before exchange", async () => {
  ...
  assert.equal(
    calls.filter(
      ({ url }) => url.toString() === "https://oauth2.googleapis.com/token",
    ).length,
    1,
  );
  const tokenBody = calls.find(
    ({ url }) => url.toString() === "https://oauth2.googleapis.com/token",
  ).init.body;
  assert.equal(tokenBody.get("code_verifier"), transaction.pkceCookie);
  assert.equal(replayChecks, 1);
  assert.equal(
    eventOrder.indexOf("replay-checked") <
      eventOrder.indexOf("persistence-started"),
    true,
  );
  ...
  assert.equal(
    JSON.stringify(persistedConnection).includes("route-test-refresh-token"),
    false,
  );
  assert.equal(
    decryptRefreshToken(
  ...
  assert.deepEqual(eventOrder.slice(-2), [
...
test("callback route rejects invalid transaction inputs before token exchange", async () => {
  for (const scenario of [
    ...
    if (scenario === "missing-state")
      request = callbackRequest(transaction, { state: null });
    if (scenario === "mismatched-state")
      request = callbackRequest(transaction, { state: "different-state" });
    if (scenario === "expired-state") {
      const expired = createOAuthStateEnvelope(
        transaction.state,
        process.env.ADMIN_SESSION_SECRET,
        Date.now() - 700_000,
      );
      request = callbackRequest(
        transaction,
        {},
        `rec_google_oauth_state=${expired}; rec_google_oauth_pkce=${transaction.pkceCookie}`,
      );
    }
    if (scenario === "missing-pkce")
      request = callbackRequest(
        transaction,
        {},
        `rec_google_oauth_state=${transaction.stateCookie}`,
      );
    if (scenario === "invalid-pkce")
      request = callbackRequest(
        transaction,
        {},
        `rec_google_oauth_state=${transaction.stateCookie}; rec_google_oauth_pkce=short`,
      );
    ...
    if (scenario === "missing-pkce" || scenario === "invalid-pkce")
      assert.equal(replayChecks, 1);
...
test("callback route sanitizes every specified provider, identity, calendar, crypto, and database failure", async () => {
  const scenarios = [
    [
      "access denied",
      { query: { error: "access_denied", code: null } },
    ...
    [
      "invalid identity",
      { google: { invalidIdentityToken: true } },
      "connection_failed",
    ],
    [
    ...
    [
      ...
      { google: { missingRefreshToken: true } },
      "missing_refresh_token",
    ],
    [
      "CalendarList",
      { google: { calendarFailure: true } },
      "connection_failed",
    ],
    [
      "missing primary calendar",
      { google: { noPrimary: true } },
      "connection_failed",
    ],
  ...
  for (const [name, setup, expectedCode] of scenarios) {
    ...
    const existing = {
      id: "primary",
      googleSubject: "existing",
      refreshTokenEncrypted: "existing-ciphertext",
    };
    persistedConnection = existing;
    if (setup.invalidEncryptionKey)
      process.env.GOOGLE_TOKEN_ENCRYPTION_KEY = "invalid";
    if (setup.databaseFailure)
      persistenceError = new Error("sensitive database failure");
    const transaction = transactionFrom(await connectRoute());
    const calls = installGoogleFetch(setup.google);
    const response = await callbackRoute(
      callbackRequest(transaction, setup.query),
    );
    ...
    if (name === "access denied" || name === "missing code")
