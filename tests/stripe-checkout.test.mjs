...
function fixture(overrides = {}) {
  ...
  const session = {
    ...
    expires_at: Math.floor(now.getTime() / 1000) + 1920,
  };
  const calls = [];
  const persistence = {
...
test("canonical listening session product is 55 minutes and exactly GBP 5500", () => {
  assert.deepEqual(SESSION_PRODUCT, {
    durationMinutes: 55,
    amountMinor: 5500,
    currency: "gbp",
    displayPrice: "£55",
...
test("active hold creates and persists a correlated card-only Checkout Session", async () => {
  ...
  const result = await createBookingCheckout(
    { bookingId, amount: 1, currency: "usd" },
    now,
    { ...f, appUrl: "https://example.test", getNow: () => now },
  );
  assert.equal(result.url, f.session.url);
  assert.equal(f.getAttached().stripeCheckoutSessionId, f.session.id);
  assert.equal(
    f.getAttached().expiresAt.toISOString(),
  ...
  assert.equal(
    successUrl.searchParams.get("session_id"),
    "{CHECKOUT_SESSION_ID}",
  );
  for (const forbidden of [
    ...
    assert.equal(successUrl.searchParams.has(forbidden), false);
...
test("invalid, expired, and non-HOLD bookings cannot start Checkout", async () => {
  await assert.rejects(
    () => createBookingCheckout({ bookingId: "bad" }, now, {}),
    InvalidCheckoutRequestError,
  );
  for (const booking of [
    ...
    await assert.rejects(
      () =>
        createBookingCheckout({ bookingId }, now, {
          ...f,
...
test("retry retrieves the one persisted open Session", async () => {
  ...
  const result = await createBookingCheckout({ bookingId }, now, {
    ...f,
    appUrl: "https://example.test",
    getNow: () => now,
  });
...
test("concurrent initiation exposes one Stripe Session and reconciles one database association", async () => {
  ...
  f.persistence.findBooking = async () =>
    associated
      ? {
          ...f.booking,
          stripeCheckoutSessionId: f.session.id,
          expiresAt: new Date(f.session.expires_at * 1000),
        }
      : f.booking;
  ...
  assert.equal(
    new Set(f.calls.map(({ options }) => options.idempotencyKey)).size,
    1,
  );
});
  ...
  await assert.rejects(
    () =>
      createBookingCheckout({ bookingId }, now, {
        ...f,
        appUrl: "https://example.test",
        getNow: () => new Date(now.getTime() + 16 * 60_000),
      }),
    HoldUnavailableError,
  );
...
test("ambiguous persistence failure reuses the same Stripe idempotency key on retry", async () => {
  ...
  assert.equal(
    f.calls[0].options.idempotencyKey,
    f.calls[1].options.idempotencyKey,
  );
});
