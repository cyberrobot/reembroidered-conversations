...
const databaseConfigured = Boolean(schemaTestUrl && applicationUrl && schemaTestUrl === applicationUrl);
const databaseSkipReason = !schemaTestUrl || !applicationUrl
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const initial = await db.booking.findUniqueOrThrow({ where: { id: bookingId } });
      ...
      const stripe = {
        checkout: {
          sessions: {
            ...
            expire: async () => assert.fail('the successful Checkout Session must not be expired'),
          },
        },
      };
      ...
      assert.equal(stripeCreateCalls[0].options.idempotencyKey, `booking-checkout:${bookingId}`);

      const withCheckout = await db.booking.findUniqueOrThrow({ where: { id: bookingId } });
      ...
      await processStripeWebhookEvent(
        ...
        async (paidBooking) => {
          const persistedPaid = await db.booking.findUniqueOrThrow({ where: { id: bookingId } });
          assert.equal(persistedPaid.id, initial.id);
          assert.equal(persistedPaid.status, 'PAID');
          assert.equal(persistedPaid.stripeCheckoutSessionId, checkoutSessionId);
          ...
          return reconcileBookingCalendarEvent(paidBooking, {
            ...
            insertEvent: async ({ accessToken, calendarId, event }) => {
              ...
              assert.equal(event.extendedProperties.private.bookingId, bookingId);
