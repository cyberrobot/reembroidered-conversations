...
const availability = {
  ...
  days: days.map(([date, first, second]) => ({
    ...
    slots: [first, second].map((time) => ({
      startAt: `${date}T${time}`,
      endAt: new Date(Date.parse(`${date}T${time}`) + 55 * 60_000).toISOString(),
    })),
  })),
};
...
async function openBookingPicker(page: Page) {
  ...
  await page.addStyleTag({
    content: '#main-nav { display: none !important; } *, *::before, *::after { animation: none !important; transition: none !important; }',
  });
  return form;
}
...
for (const viewport of [
  ...
  test(`homepage booking picker preserves the ${viewport.name} baseline`, async ({ page }) => {
    await page.setViewportSize(viewport);
    const form = await openBookingPicker(page);
    await expect(form).toHaveScreenshot(`homepage-booking-picker-${viewport.name}.png`, {
      animations: 'disabled',
    });
  });
}

test('homepage picker preserves date ordering, badges, end card, and controlled selections', async ({ page }) => {
  ...
  expect(await cards.evaluateAll((elements) => elements.map((element) => element.getAttribute('data-availability-date'))))
    .toEqual(days.map(([date]) => date));
  await expect(cards.first()).toContainText('Nearest');
  await expect(cards.nth(1).getByText('Nearest', { exact: true })).toHaveCount(0);
  ...
  await expect(page.getByRole('dialog', { name: 'Select a specific date' })).toBeVisible();
  ...
  await expect(form.getByRole('button', { name: '9:00 AM' })).toHaveAttribute('aria-pressed', 'true');
  await form.getByRole('button', { name: '12:00 PM' }).click();
  await expect(form.getByRole('button', { name: '12:00 PM' })).toHaveAttribute('aria-pressed', 'true');
  await expect(form.getByRole('button', { name: '9:00 AM' })).toHaveAttribute('aria-pressed', 'false');
});

test('full-calendar later-date selection scrolls its date card into the visible strip', async ({ page }) => {
  await page.setViewportSize({ width: 768, height: 1024 });
  const form = await openBookingPicker(page);
  const strip = form.getByRole('group', { name: 'Available dates' });
  const targetDate = days.at(-1)![0];
  const targetCard = strip.locator(`button[data-availability-date="${targetDate}"]`);
  ...
  await expect(form.getByText('Available times on Wednesday, 31 Jan:')).toBeVisible();
