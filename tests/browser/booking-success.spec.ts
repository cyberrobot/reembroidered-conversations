...
async function seedBooking(index: number, status: 'PAID' | 'CONFIRMED' = 'CONFIRMED', email = 'sarah@example.test') {
  ...
  await pool!.query(
    ...
    [id, status, sessionId, status === 'CONFIRMED' ? `rec${id.replaceAll('-', '')}` : null, status === 'CONFIRMED' ? 'https://meet.google.com/abc-defg-hij' : null, email],
  );
  return { id, sessionId };
}
...
test('customer can later open the known success URL for a webhook-confirmed booking without a Stripe redirect', async ({ page, context }) => {
  ...
  await page.goto(`/booking/success?booking_id=${fixture.id}&session_id=${fixture.sessionId}&name=Attacker&email=attacker@example.test&amount=£1&meetingUrl=https://example.com`);

  await expect(page.getByRole('heading', { level: 1 })).toHaveText('Your conversation with Shahd Karaeen is reserved.');
  for (const text of ['Sarah Jenkins', fixture.id, 'Thursday, 24 September 2026', '14:00–14:55 (BST) · 55 min', 'Private video meeting via Google Meet', 'sarah@example.test', '£55 paid']) await expect(page.getByText(text, { exact: false })).toBeVisible();
  ...
  await expect(page.locator('#main-nav')).not.toContainText('Return to Re-Embroidered');
  ...
  expect(await page.evaluate(() => navigator.clipboard.readText())).toBe(fixture.id);
...
test('confirmed mobile layout has no horizontal overflow', async ({ page }) => {
  const fixture = await seedBooking(0, 'CONFIRMED', 'sarah.jenkins.with.a.long.booking.address@example.test');
  ...
  await expect(page.getByText('sarah.jenkins.with.a.long.booking.address@example.test')).toBeVisible();
