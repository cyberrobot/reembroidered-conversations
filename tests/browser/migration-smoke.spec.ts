...
function collectBrowserFailures(page: Page) {
  ...
  page.on("console", (message) => {
    ...
    if (message.type() === "error" && !isExpectedInvalidMuxProbe)
...
async function mockAvailability(
  page: Page,
  body: unknown = availabilityFixture(),
  status = 200,
) {
...
test("production routes ignore legacy payment-authority query parameters", async ({
  request,
}) => {
  await expect((await request.get("/")).status()).toBe(200);
  const legacyConfirmation = await request.get(
    "/confirmation?payment_success=true",
    { maxRedirects: 0 },
  );
  expect(legacyConfirmation.status()).toBe(307);
  expect(
    new URL(legacyConfirmation.headers().location!, "http://127.0.0.1")
      .pathname,
  ).toBe("/booking/success");
  ...
  for (const [key, value] of entries) {
    ...
    expect(
      response.status(),
      `${key} must not manufacture a confirmation redirect`,
    ).toBe(200);
  }
});
...
test("homepage, navigation, Mux fallback, and transcript work without browser errors", async ({
  ...
  const playControl = page.getByRole("button", {
    name: "Play video introduction from Shahd Karaeen",
  });
...
test("mobile navigation opens, closes, and navigates", async ({ page }) => {
  ...
  await toggle.click();
  await page
    .getByRole("button", { name: "Reserve a Conversation (£55)" })
    .click();
  await expect(
    page.getByRole("button", { name: "Meet Shahd & Video" }),
  ).toBeHidden();
...
test("one booking action creates a hold and automatically redirects to Checkout", async ({
  page,
}, testInfo) => {
  const failures = collectBrowserFailures(page);
  await page.setViewportSize({ width: 1280, height: 720 });
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        hold: {
          ...
          endAt: new Date(
            Date.parse(String(submittedHold!.startAt)) + 55 * 60_000,
          ).toISOString(),
          timezone: "Europe/London",
  ...
  const dateCards = page
    .locator("#book-session form button")
    .filter({ hasText: /slots?/ });
  ...
  expect(new Set(dayPrefixes)).toEqual(
  ...
  await expect(page.getByRole("button", { name: "5:00 PM" })).toHaveClass(
    /bg-\[#282524\]/,
  );
  ...
  const initialMonth = await nextMonth.evaluate((button) =>
    button.previousElementSibling?.textContent?.trim(),
  );
  await nextMonth.click();
  await expect
    .poll(() =>
      nextMonth.evaluate((button) =>
        button.previousElementSibling?.textContent?.trim(),
      ),
    )
    .not.toBe(initialMonth);
  await dialog.getByRole("button", { name: "Previous month" }).click();
  const availableCalendarDate = dialog
    .locator('button[title*="available slots"]')
    .nth(1);
  const selectedCalendarTitle =
    await availableCalendarDate.getAttribute("title");
  await availableCalendarDate.click();
  await expect(dialog).toBeHidden();
  await expect(
    page.getByText(
      `Available times on ${selectedCalendarTitle!.split(" (")[0]}:`,
    ),
  ).toBeVisible();
  ...
  expect(
    await name.evaluate((input: HTMLInputElement) => input.validationMessage),
  ).not.toBe("");
  ...
  await page.locator("#confirm-booking-button").click();
  await expect(
    page.getByText(/Please confirm that you understand/),
  ).toBeVisible();
  ...
  expect(submittedCheckout).toEqual({
...
test("lost-slot and temporary hold failures preserve form state and allow recovery", async ({
  ...
  await page.route("**/api/bookings/hold", (route) => {
    ...
    return route.fulfill({
      ...
      body: JSON.stringify({
        error: {
          code: holdRequests === 1 ? "slot_unavailable" : "hold_unavailable",
        },
      }),
    });
  });
  ...
  await page.locator("#confirm-booking-button").click();
  await expect(
    page.getByRole("alert").filter({ hasText: "has not yet been reserved" }),
  ).toBeVisible();
  await expect(page.locator("#client-name")).toHaveValue("Preserved Name");
...
test("checkout initiation failure is retryable and a lost hold refreshes availability", async ({
  page,
}) => {
  let availabilityRequests = 0;
  await page.route("**/api/availability*", (route) => {
    ...
    return route.fulfill({
      ...
      body: JSON.stringify({
        hold: {
          ...
          endAt: "2099-01-02T10:55:00.000Z",
          timezone: "Europe/London",
          expiresAt: new Date(Date.now() + 15 * 60_000).toISOString(),
        },
      }),
  ...
  await expect(
    page.getByRole("button", { name: "View full calendar" }),
  ).toBeDisabled();
  expect(holdRequests).toBe(1);
  await page.locator("#confirm-booking-button").press("Enter");
  expect(holdRequests).toBe(1);
  await expect(
    page.getByRole("alert").filter({ hasText: "no longer reserved" }),
  ).toBeVisible();
  await expect.poll(() => availabilityRequests).toBeGreaterThan(1);
...
test("locally expired hold clears ownership and refreshes availability", async ({
  page,
}) => {
  let availabilityRequests = 0;
  await page.route("**/api/availability*", (route) => {
  ...
  await page.locator("#confirm-booking-button").click();
  await expect(
    page.getByRole("alert").filter({ hasText: "still held" }),
  ).toBeVisible();
  await expect(page.locator("#client-name")).toBeDisabled();
  await expect(
    page.getByRole("alert").filter({ hasText: "temporary hold has expired" }),
  ).toBeVisible();
  await expect.poll(() => availabilityRequests).toBeGreaterThan(1);
  await expect(page.locator("#client-name")).toBeEnabled();
});

test("a pending Checkout response cannot affect state after its hold expires", async ({
  page,
}) => {
  let availabilityRequests = 0;
  await page.route("**/api/availability*", (route) => {
    ...
    return route.fulfill({
      ...
      body: JSON.stringify({
        hold: {
          ...
          timezone: "Europe/London",
          expiresAt: new Date(
            Date.now() + (holdRequests === 1 ? 750 : 15 * 60_000),
          ).toISOString(),
        },
      }),
    });
  ...
  const expiredAlert = page
    .getByRole("alert")
    .filter({ hasText: "temporary hold has expired" });
...
test("booking availability distinguishes loading, empty, and recoverable service errors", async ({
  page,
}) => {
  ...
  await page.route("**/api/availability*", async (route) => {
    ...
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ timezone: "Europe/London", days: [] }),
    });
  });
  await page.goto("/");
  ...
  await page.reload();
  await expect(
    page.getByRole("alert").filter({ hasText: "We could not load" }),
  ).toContainText("We could not load availability just now");
  await expect(page.getByRole("button", { name: "Try again" })).toBeVisible();
  await expect(page.locator("#confirm-booking-button")).toBeDisabled();
});

test("payment return requires matching booking and Checkout Session identifiers", async ({
  page,
}) => {
  const failures = collectBrowserFailures(page);
  ...
  try {
    ...
    await expect(page).toHaveURL(
      new RegExp(`/booking/success\\?booking_id=${paidId}`),
    );
    await expect(
      page.getByText("We couldn’t verify this booking link."),
    ).toBeVisible();
    ...
    await page.goto(
      `/payment?payment_success=true&success=true&booking_id=${paidId}&session_id=cs_test_wrong`,
    );
    await expect(
      page.getByText("We couldn’t verify this booking link."),
    ).toBeVisible();
...
test("responsive visual tokens and layouts remain intact", async ({
  page,
}, testInfo) => {
  const viewports = [
  ...
  for (const viewport of viewports) {
    ...
    const visualTokens = await page.evaluate(() => {
      const pageRootStyle = getComputedStyle(
        document.querySelector<HTMLElement>(".paper-grain")!,
      );
      const headingStyle = getComputedStyle(document.querySelector("h1")!);
      const aboutImage =
        document.querySelector<HTMLImageElement>("#about-shahd img")!;
