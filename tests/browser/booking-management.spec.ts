...
test.skip(
  !connectionString,
  "DATABASE_URL is required for booking management browser coverage",
);
...
async function seed(
  ...
  await pool!.query(
    ...
    [
      ...
      endAt,
      status,
      `cs_management_${index}`,
      `pi_management_${index}`,
      `event_management_${index}`,
...
test.afterEach(async () => {
  if (pool)
    await pool.query(
      'DELETE FROM "bookings" WHERE "id" = ANY($1::uuid[]) OR "rescheduleSourceBookingId" = ANY($1::uuid[])',
      [ids],
    );
});
test.afterAll(async () => {
...
test("confirmed booking renders authoritative details, secure headers, keyboard actions, and desktop visual", async ({
  page,
}) => {
  ...
  for (const text of [
    "Sarah Jenkins",
    "sarah.j@example.test",
    "Monday, 8 January 2035",
    "10:00–10:55",
    "55 min",
    "£55.00",
...
test("reschedule selection, review, processing, success, and conflict use real API results", async ({
  page,
}) => {
  const fixture = await seed(0);
  ...
  await page.route(
    `**/api/bookings/manage/${fixture.capability}/reschedule`,
    async (route) => {
  ...
  await expect(
    page.getByText("Available times on Wednesday, 10 Jan:"),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "2:00 PM" })).toHaveAttribute(
    "aria-pressed",
    "true",
  );
  ...
  await expect(
    page.getByRole("heading", {
      name: "Your conversation has been rescheduled",
    }),
  ).toBeVisible();
  await expect(
    page.getByText("Wednesday, 10 January 2035", { exact: false }),
  ).toBeVisible();
  ...
  await page.getByRole("button", { name: "Secure this new time" }).click();
  await expect(
    page.getByRole("heading", { name: "That time could not be secured" }),
  ).toBeVisible();
  await expect(
    page.getByText("remains protected", { exact: false }),
  ).toBeVisible();
...
test("cancellation is explicit, shows processing, and renders precise refund pending state", async ({
  page,
}) => {
  const fixture = await seed(0);
  let releaseResponse!: () => void;
  ...
  await page.route(
    `**/api/bookings/manage/${fixture.capability}/cancel`,
    async (route) => {
      await responseGate;
      await route.fulfill({
        json: {
          ...
          refund: { eligible: true, status: "pending" },
  ...
  await expect(
    page.getByText("eligible for a full refund", { exact: false }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Keep my booking" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "Yes, cancel booking" }).click();
  await expect(
    page.getByRole("heading", { name: "Cancelling your booking…" }),
  ).toBeVisible();
  releaseResponse();
  await expect(
...
test("provider error detail remains visible after returning to cancellation confirmation", async ({
  page,
}) => {
  const fixture = await seed(0);
  await page.route(
      ...
      route.fulfill({
        ...
        json: {
          error: {
            ...
            message:
              "Cancellation is temporarily unavailable. Please try again.",
          },
        },
      }),
  );
...
test("stale refundable policy is updated and requires explicit confirmation again", async ({
  page,
}) => {
  const fixture = await seed(0);
  let attempts = 0;
  ...
  await expect(
    page.getByText("automatic refund period has passed", { exact: false }),
  ).toBeVisible();
...
test("refreshing an uncertain reschedule exposes and resumes the exact linked hold", async ({
  page,
}) => {
  const fixture = await seed(0);
  await pool!.query(
    ...
    [
      ids[3],
      "2035-01-09T10:00:00.000Z",
      "2035-01-09T10:55:00.000Z",
      "2034-12-01T10:15:00.000Z",
      fixture.id,
    ],
  );
  ...
  await page.goto(`/booking/manage/${fixture.capability}`);
  await expect(
    page.getByRole("heading", { name: "Your change is still being finalised" }),
  ).toBeVisible();
  await expect(page.getByText("Requested time")).toBeVisible();
  await page
    .getByRole("button", { name: "Continue finalising this change" })
    .click();
  await expect(
    page.getByRole("heading", {
      name: "Your conversation has been rescheduled",
...
test("late cancellation copy promises no automatic refund", async ({
  page,
}) => {
...
test("invalid capability reveals no booking data and has a protected visual", async ({
  page,
}) => {
  await seed(0);
...
test("cancelled/refunded booking is inactive and accurately reports provider state", async ({
  page,
}) => {
...
test("mobile confirmed and reschedule layouts have no horizontal overflow", async ({
  page,
}) => {
  const fixture = await seed(0);
  await page.route("**/api/availability", (route) =>
