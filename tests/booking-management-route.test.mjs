...
test("management state responses are no-store and invalid links disclose no booking data", async () => {
  const valid = createManagementStateHandler(async () => ({
    kind: "active",
    booking: { name: "Listener" },
  }));
  const validResponse = await valid(
    request("/api/bookings/manage/token"),
    "token",
  );
  ...
  const invalid = createManagementStateHandler(async () => ({
    kind: "invalid",
    private: "must-not-leak",
  }));
  const invalidResponse = await invalid(
    request("/api/bookings/manage/tampered"),
    "tampered",
  );
  assert.equal(invalidResponse.status, 404);
  assert.equal(
    JSON.stringify(await invalidResponse.json()).includes("private"),
    false,
  );
});
...
test("cancellation requires capability authority and same-origin mutation", async () => {
  ...
  const invalid = await handler(
    request("/api/bookings/manage/invalid/cancel", cancellationRequest),
    "invalid",
  );
  assert.equal(invalid.status, 404);
  assert.equal(calls, 0);
  const crossOrigin = await handler(
    request("/api/bookings/manage/valid/cancel", {
      ...cancellationRequest,
      headers: {
        ...cancellationRequest.headers,
...
test("same-origin mutation accepts the browser-facing host when Next uses an internal bind address", async () => {
  ...
  const response = await handler(
    new Request("http://0.0.0.0:3000/api/bookings/manage/valid/reschedule", {
      method: "POST",
      headers: {
...
test("same-origin mutation accepts a trusted proxy-facing authority but still rejects another origin", async () => {
  ...
  const valid = await handler(
    new Request("http://app:3000/api/bookings/manage/valid/reschedule", {
      method: "POST",
      headers: { ...headers, Origin: "https://bookings.example.test" },
      body: '{"startAt":"2030-01-01T10:00:00.000Z"}',
    }),
    "valid",
  );
...
test("reschedule rejects malformed JSON and maps sanitized stable errors", async () => {
  ...
  const handler = createRescheduleHandler(
    () => "booking-id",
    async (_bookingId, body) => {
      input = body;
      throw new BookingRescheduleError("slot_unavailable");
    },
  );
  const malformed = await handler(
    request("/api/bookings/manage/valid/reschedule", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: "{",
    }),
