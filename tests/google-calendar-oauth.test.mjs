...
test("OAuth scopes are exactly the approved identity, CalendarList, FreeBusy, and owned-event scopes", () => {
  assert.deepEqual(
    [...GOOGLE_OAUTH_SCOPES],
    [
      "openid",
...
test("OAuth state is random, signed, matched, and expires", () => {
  ...
  assert.equal(
    validateOAuthStateEnvelope(envelope, second, secret, now),
    false,
  );
  assert.equal(
    validateOAuthStateEnvelope(undefined, first, secret, now),
    false,
  );
  assert.equal(
    validateOAuthStateEnvelope(envelope, first, secret, now + 601_000),
    false,
  );
});
