...
test("Google identity token validation verifies signature, issuer, audience, and expiry", async () => {
  ...
  await assert.rejects(() =>
    verifyGoogleIdentityToken(
      createToken({ iss: "https://attacker.invalid" }),
      "client-id",
      mockFetch,
      now,
    ),
  ...
  await assert.rejects(() =>
    verifyGoogleIdentityToken(
      createToken({ exp: Math.floor(now / 1000) }),
      "client-id",
      mockFetch,
      now,
    ),
  ...
  await assert.rejects(() =>
    verifyGoogleIdentityToken(
      createToken({}, otherKeys.privateKey),
      "client-id",
      mockFetch,
      now,
    ),
...
test("admin session is signed, finite, and rejects tampering", () => {
  ...
  assert.equal(
    validateAdminSession(session, secret, now + 8 * 60 * 60 * 1000 + 1_000),
    null,
  );
