...
const source = (overrides = {}) => ({
  ...
  startAt: new Date("2030-01-08T10:00:00.000Z"),
  endAt: new Date("2030-01-08T10:55:00.000Z"),
  timezone: "Europe/London",
  status: "CONFIRMED",
  stripeCheckoutSessionId: "cs_existing",
  stripePaymentIntentId: "pi_existing",
  calendarEventId: "event_existing",
...
const target = {
  date: "2030-01-09",
  startAt: "2030-01-09T10:00:00.000Z",
  endAt: "2030-01-09T10:55:00.000Z",
};
  ...
  const persistence = {
    ...
    reserveTarget: async ({ source: booking, target: selected }) => {
      ...
      pending = {
        id: "target-hold",
        status: "HOLD",
        rescheduleSourceBookingId: booking.id,
  ...
  return {
    dependencies: {
      ...
      getAvailableSlots: async () =>
        options.available === false ? [] : [target],
      rescheduleCalendarEvent: async (_booking, hold) => {
        ...
        if (options.calendarService)
          return options.calendarService(_booking, hold);
        if (options.calendarError) throw options.calendarError;
        return {
          calendarEventId: "event_existing",
          meetingUrl:
            options.meetingUrl ?? "https://meet.google.com/abc-defg-hij",
        };
      },
...
function calendarServiceWithGetFailure(category) {
  return (booking, hold) =>
    rescheduleBookingCalendarEvent(booking, hold, {
      ...
      updateEvent: async () => {
        throw new Error("PATCH must not be attempted");
      },
    });
}
...
test("unavailable, same-slot and competing targets leave the original booking unchanged", async () => {
  ...
  assert.equal(
    unavailable.source().startAt.toISOString(),
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof BookingRescheduleError &&
      error.code === "invalid_reschedule_request",
  );
  ...
  const pending = fixture({
    pending: {
      ...
      endAt: new Date("2030-01-10T10:55:00.000Z"),
      timezone: "Europe/London",
    },
  });
  await assert.rejects(
...
test("definite Calendar failure releases the target while uncertain failure retains both reservations", async () => {
  for (const category of ["unavailable", "not_found", "invalid_response"]) {
    const definite = fixture({
      calendarService: calendarServiceWithGetFailure(category),
    });
    await assert.rejects(
      () =>
        rescheduleBooking(
  ...
  const uncertain = fixture({
    calendarError: new CalendarManagementError("provider_unavailable"),
  });
  await assert.rejects(
    () =>
      rescheduleBooking(
...
test("malformed PATCH results and post-PATCH validation failures retain the target hold", async () => {
  for (const code of ["invalid_provider_response", "event_mismatch"]) {
    const attempt = fixture({
      calendarError: new CalendarManagementError(code, {
        outcome: "uncertain",
      }),
    });
    await assert.rejects(
