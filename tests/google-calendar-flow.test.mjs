...
for (const [name, overrides, expectedCode] of [
  ...
  [
    ...
    {
      discoverCalendar: async () => {
        throw new Error("raw calendar detail");
      },
    },
    "connection_failed",
  ],
...
test("CalendarList discovery follows pagination and selects only primary metadata", async () => {
  ...
  const calendar = await discoverPrimaryCalendar(
    "access-token",
    async (url, options) => {
  ...
  assert.equal(
    new URL(requests[1].url).searchParams.get("pageToken"),
    "page-2",
  );
  assert.equal(
...
test("CalendarList discovery fails rather than selecting a secondary calendar", async () => {
  await assert.rejects(() =>
    discoverPrimaryCalendar("token", async () => ({
      ok: true,
      json: async () => ({
        items: [{ id: "secondary", summary: "Secondary" }],
      }),
  ...
  await assert.rejects(() =>
    completeGoogleOAuth(
      ...
      successfulDependencies({
        ...
        persistConnection: async (replacement) =>
          Object.assign(existingConnection, replacement),
      }),
    ),
  );
