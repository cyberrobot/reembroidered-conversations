...
export function createBookingHoldHandler(service: HoldService, getNow = () => new Date()) {
  return async function bookingHoldHandler(request: Request) {
    ...
    try {
      ...
      return NextResponse.json(
        { error: { code: 'invalid_hold_request', message: 'Check your booking details and try again.' } },
        { status: 400, headers },
      );
    }
