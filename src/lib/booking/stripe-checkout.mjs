...
export class HoldUnavailableError extends Error {
  constructor() {
    super("Booking hold unavailable.");
...
export class CheckoutUnavailableError extends Error {
  /** @param {unknown} [cause] */
  constructor(cause) {
    super("Checkout unavailable.", { cause });
...
function validCheckoutUrl(url) {
  if (!url) return false;
  try {
    return new URL(url).origin === "https://checkout.stripe.com";
  } catch {
    return false;
  }
}
...
export function createCheckoutPersistence(database) {
  return {
    ...
    attachSession: ({ bookingId, sessionId, expiresAt, now }) =>
...
export async function createBookingCheckout(
  ...
  if (!appUrl)
    throw new CheckoutUnavailableError(new Error("APP_URL is required."));

  const booking = await persistence.findBooking(bookingId);
  if (!booking || booking.status !== "HOLD" || booking.expiresAt <= now)
  ...
  if (booking.stripeCheckoutSessionId) {
    try {
      ...
      if (
        ...
        existing.expires_at * 1000 > now.getTime() &&
        validCheckoutUrl(existing.url)
      ) {
        return {
          url: existing.url,
  ...
  try {
    const result = await persistence.attachSession({
      bookingId,
    ...
    if (result.count === 1)
      return { url: session.url, expiresAt: effectiveExpiry };
    const reconciled = await persistence.findBooking(bookingId);
    if (
      reconciled?.status === "HOLD" &&
      reconciled.expiresAt > commitNow &&
      reconciled.stripeCheckoutSessionId === session.id
    ) {
  ...
  try {
    ...
    /* webhook/reconciliation can retry cleanup */
  }
  throw new HoldUnavailableError();
}
