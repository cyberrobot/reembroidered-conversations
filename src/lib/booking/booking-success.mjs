...
export function formatConfirmedBooking(booking) {
  ...
  try {
    return {
      ...
      timezoneName: formatInTimeZone(startAt, booking.timezone, {
        timeZoneName: "short",
      })
        .split(" ")
        .at(-1),
...
export async function getBookingSuccessState(input, dependencies = {}) {
  if (
    ...
    !CHECKOUT_SESSION_PATTERN.test(input.sessionId)
  )
    return { kind: "invalid" };
  ...
  let booking;
  try {
    booking = await persistence.findBooking(input.bookingId);
  } catch {
    return { kind: "unavailable" };
  }
  if (!booking || booking.stripeCheckoutSessionId !== input.sessionId)
    return { kind: "invalid" };
