...
export class StripeWebhookReconciliationError extends Error {
   ...
   */
  constructor(
    message = "Stripe event could not be reconciled.",
...
export function createStripeWebhookPersistence(database) {
  return {
    markPaid: ({ bookingId, sessionId, paymentIntentId }) =>
    ...
    cancelExpired: ({ bookingId, sessionId }) =>
      database.booking.updateMany({
        where: {
          id: bookingId,
          status: "HOLD",
    ...
    recordConfirmationEmail: ({ bookingId, messageId, acceptedAt }) =>
      database.booking.updateMany({
        where: {
          id: bookingId,
          status: "CONFIRMED",
...
export function checkoutSessionBookingId(session) {
  ...
  if (
    ...
    session.client_reference_id !== metadataId
  ) {
...
export async function reconcileBookingConfirmationEmail(
  ...
  let result;
  try {
    result = await sendConfirmationEmail(booking);
  } catch (error) {
    const deliveryCode =
      {
...
export async function processStripeWebhook(event) {
  ...
  const { reconcileBookingCalendarEvent } =
