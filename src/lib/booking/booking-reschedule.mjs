...
import { CalendarManagementError, rescheduleBookingCalendarEvent } from '../calendar/booking-event.mjs';
import { BOOKING_HOLD_MINUTES, isActiveSlotUniqueConflict } from './booking-hold.mjs';
...
const sourceSelection = {
  id: true, name: true, email: true, startAt: true, endAt: true, timezone: true,
  status: true, stripeCheckoutSessionId: true, stripePaymentIntentId: true,
  calendarEventId: true, meetingUrl: true, rescheduledAt: true,
};
const holdSelection = {
  id: true, startAt: true, endAt: true, timezone: true, status: true,
  rescheduleSourceBookingId: true,
};

export function createBookingReschedulePersistence(database) {
  const findPendingHold = (bookingId) => database.booking.findUnique({
    where: { rescheduleSourceBookingId: bookingId }, select: holdSelection,
  });
  return {
    findSource: (bookingId) => database.booking.findUnique({ where: { id: bookingId }, select: sourceSelection }),
    findPendingHold,
    reserveTarget: async ({ source, target, now }) => {
      try {
        return await database.$transaction(async (transaction) => {
          const current = await transaction.booking.findUnique({ where: { id: source.id }, select: sourceSelection });
          if (!current || current.status !== 'CONFIRMED' || current.startAt <= now ||
          ...
          const pending = await transaction.booking.findUnique({
            where: { rescheduleSourceBookingId: source.id }, select: holdSelection,
          });
          if (pending) {
            if (pending.status === 'HOLD' && pending.startAt.getTime() === target.startAt.getTime()) return pending;
        ...
        if (isActiveSlotUniqueConflict(error)) throw new BookingRescheduleError('slot_unavailable');
        const pending = await findPendingHold(source.id);
        if (pending?.status === 'HOLD') {
          if (pending.startAt.getTime() === target.startAt.getTime()) return pending;
          throw new BookingRescheduleError('change_in_progress');
        }
        throw error;
      }
    },
    releaseTarget: ({ sourceId, holdId }) => database.booking.updateMany({
      where: { id: holdId, status: 'HOLD', rescheduleSourceBookingId: sourceId },
      data: { status: 'CANCELLED', rescheduleSourceBookingId: null },
    }),
    commitTarget: ({ source, hold, meetingUrl, now }) => database.$transaction(async (transaction) => {
      const currentHold = await transaction.booking.findUnique({ where: { id: hold.id }, select: holdSelection });
      if (!currentHold || currentHold.status !== 'HOLD' || currentHold.rescheduleSourceBookingId !== source.id ||
      ...
      const retired = await transaction.booking.updateMany({
        where: { id: hold.id, status: 'HOLD', rescheduleSourceBookingId: source.id },
        data: { status: 'CANCELLED', rescheduleSourceBookingId: null },
      });
      if (retired.count !== 1) throw new BookingRescheduleError('reconciliation_pending');
      const changed = await transaction.booking.updateMany({
        where: { id: source.id, status: 'CONFIRMED', startAt: source.startAt },
        data: {
          startAt: hold.startAt, endAt: hold.endAt, timezone: hold.timezone,
          meetingUrl, rescheduledAt: now,
        },
      });
      if (changed.count !== 1) throw new BookingRescheduleError('reconciliation_pending');
      return transaction.booking.findUniqueOrThrow({ where: { id: source.id }, select: sourceSelection });
    }),
  };
}

function parseRequestedStart(value) {
  if (typeof value !== 'string' || !ISO_INSTANT.test(value)) throw new BookingRescheduleError('invalid_reschedule_request');
...
export async function rescheduleBooking(bookingId, input, now, dependencies, config = PROVIDER_AVAILABILITY_CONFIG) {
  const requestedStart = parseRequestedStart(input?.startAt);
  let source;
  try { source = await dependencies.persistence.findSource(bookingId); }
  catch { throw new BookingRescheduleError('management_unavailable'); }
  ...
  if (!hold) {
    ...
    try {
      hold = await dependencies.persistence.reserveTarget({
        source,
        target: {
          startAt: new Date(canonical.startAt), endAt: new Date(canonical.endAt), timezone: config.timezone,
        },
        now,
      });
    } catch (error) {
  ...
  try {
    const booking = await dependencies.persistence.commitTarget({
      source, hold, meetingUrl: calendar.meetingUrl, now: dependencies.getNow(),
    });
...
export async function rescheduleBookingWithDefaultDependencies(bookingId, input, now = new Date()) {
