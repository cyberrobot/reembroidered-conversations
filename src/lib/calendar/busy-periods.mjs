...
const defaultDependencies = {
  getCredentials: async () => {
    const { getGoogleCalendarCredentials } = await import('../google-calendar/connection.ts');
    return getGoogleCalendarCredentials();
  },
  getOAuthConfig: async () => {
    const { getGoogleOAuthConfig } = await import('../google-calendar/config.ts');
    return getGoogleOAuthConfig();
  },
  refreshAccessToken: async (input) => {
    const { refreshGoogleAccessToken } = await import('../google-calendar/google-api.mjs');
    return refreshGoogleAccessToken(input);
  },
  queryFreeBusy: async (input) => {
    const { queryGoogleFreeBusy } = await import('../google-calendar/google-api.mjs');
 ...
 */
export async function getBusyPeriods(from, to, dependencies = defaultDependencies) {
  if (
